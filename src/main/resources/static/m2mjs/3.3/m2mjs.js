...
class Connection {
	...
	static get (url, isJson = true, isSync = false) {
		if (isSync) {
			return new Connection(url, Connection.Methods.GET, null, isJson).connectSync();
		}
	...
	static post (url, requestData, isJson = true, isSync = false) {
		if (isSync) {
			return new Connection(url, Connection.Methods.POST, requestData, isJson).connectSync();
		}
	...
	static put (url, requestData, isJson = true, isSync = false) {
		if (isSync) {
			return new Connection(url, Connection.Methods.PUT, requestData, isJson).connectSync();
		}
		
		return new Connection(url, Connection.Methods.PUT, requestData, isJson).connect();
	}

	static delete (url, requestData, isJson = true, isSync = false) {
		if (isSync) {
			return new Connection(url, Connection.Methods.DELETE, requestData, isJson).connectSync();
		}
		
		return new Connection(url, Connection.Methods.DELETE, requestData, isJson).connect();
	}
...
class XBrowser {
    ...
    static getElementByClassName(parentElement, className) {
        ...
        for (let i = 0; i < parentElement.childNodes.length; i++) {
            ...
            if (returnElement == null && parentElement.childNodes[i].childNodes.length > 0) {
                const element = XBrowser.getElementByClassName(parentElement.childNodes[i], className);
                if (element != null) {
                    return element;
                }
            }
    ...
    renderGrid() {
        ...
        this.getData().forEach (
            function (modelObject, index) {
                let gridRow = new GridRow (grid.gridTemplate, modelObject, grid.objectPaths, grid.renderersSet, grid.componentsSet);
