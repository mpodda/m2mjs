...
let personViewForm;
let myGrid;
let personValidator = new M2MJS.Validator();

async function initGrid () {
	let template = await M2MJS.TemplateLoader.loadTemplate(templatePath, "simpleGridTemplate");
...
async function renderGrid(template, dataList) {
	return new Promise((resolve, reject) => {
		myGrid = new M2MJS.Grid(dataList, document.getElementById("personDataContainer"), template);
		
		myGrid.renderers = [
			{
				"name": "showPersonInfo",
				"code" : async (element, objectValue, gr) => {
					M2MJS.XBrowser.addHandler(element, "click", async (e) => {
...
async function createPerson () { 
	return new Promise((resolve, reject) => {
		M2MJS.Connection.get(createPersonUrl).then((response) => {
...
async function loadEditFormTemplate () {
	return new Promise((resolve, reject) => {
		M2MJS.TemplateLoader.loadTemplate(formTemplatePath, "editPersonTemplate").then((response) => {
			resolve(response);
		})
	});
}
...
async function loadViewFormTemplate () {
	return new Promise((resolve, reject) => {
		M2MJS.TemplateLoader.loadTemplate(formTemplatePath, "viewPersonTemplate").then((response) => {
			resolve(response);
		})
	});
}
...
async function renderEditForm(person) {
	await personValidator.clearErrors().then (personForm.modelObject = await loadPerson(person));
}
...
async function renderViewForm(person) {
	personViewForm.modelObject = await loadPerson(person);
}
...
async function initEditForm() {
	return new Promise( async (resolve, reject) => {
		...
		let nationalities = await loadNationalities();
		
		document.getElementById("formPlaceHolder").appendChild(formTemplate);
		
		personForm = new M2MJS.Form();
		
		personForm.addComponent(new M2MJS.Component("id", document.getElementById("personId")));
		personForm.addComponent(new M2MJS.Component("name", document.getElementById("personName")));
		...
		personForm.addComponent(new M2MJS.Component("comments", document.getElementById("personComments")));
...
async function initViewForm() {
	return new Promise( async (resolve, reject) => {
		...
		personViewForm = new M2MJS.Form();
		
		personViewForm.addComponent(new M2MJS.Component("id", document.getElementById("personIdView")));
		personViewForm.addComponent(new M2MJS.Component("name", document.getElementById("personNameView")));
		...
		personViewForm.addComponent(new M2MJS.Component("comments", document.getElementById("personCommentsView")));
...
async function initEvents() {
	M2MJS.XBrowser.addHandler(document.getElementById("btnAddPerson"), "click", async (e) => {
...
function defineValidations() {
	...
	/* Name */
    let pNameField = new M2MJS.Validator.Field();
	...
	/* Gender */
    let pGenderField = new M2MJS.Validator.Field();
	...
	/* Nationality */
    let pNationalityField = new M2MJS.Validator.Field();
    ...
    /* Active */
    let pActiveField = new M2MJS.Validator.Field();
...
async function savePerson (person) {
	...
	if (isValid) {
		return new Promise((resolve, reject) => {
			M2MJS.Connection.post(savePersonUrl, person).then((response) => {
				resolve (response.data);
			})
		});
	} else {
...
M2MJS.XBrowser.addHandler(document.getElementById("btnSave"), "click", (e) => savePersonEvent());

async function savePersonEvent (e) {
	await savePerson(personForm.modelObject).then (
		async (person) => {
			myGrid.dataList = await loadPersons();
		}
	);
